...
$G: \{0,1\}^S\to \{0,1\}^n$ seed $S\ge\ge n$
...
$$
\exists i \space G(k) [1, \dots\dots, i-1] \to G(k) [1, \dots\dots, n]
$$
...
как Ева по данным первых бит достраивает генератор и находит открытый текст
...
```
 ...
 Определение предсказуемого генератор 
